...
class TaskAssigneeMail extends Mailable
{
    ...
    {
        $tamplate = EmailTemplate::where('type', 'task_assign')->first();

        $subject= $tamplate->subject;
        $body = $tamplate->value;
        
        $key = ['{ASSIGNED_FROM}','{TASK_NAME}','http://{TASK_URL}','{ASSIGNED_TO}','{DUE_DATE}','{CASE_NAME}','http://{CASE_URL}','{EMAIL_SIGNATURE}'];
